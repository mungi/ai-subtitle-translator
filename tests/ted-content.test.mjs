...
test("TED pages use player data, the native control bar, and TED caption messages", async () => {
  ...
  assert.match(contentScript, /function getTedStructuredVideoData\(talkSlug\)/);
  assert.match(contentScript, /script\[type="application\/ld\+json"\]/);
  assert.match(contentScript, /node\?\.contentUrl/);
  assert.match(contentScript, /let tedPlayerContextCache = null/);
  assert.match(contentScript, /tedPlayerContextCache\.slug !== talkSlug/);
  assert.match(contentScript, /embeddedManifestUrl \? null : getTedManifestResourceUrl\(\)/);
  ...
  assert.match(contentScript, /getSessionKey: \(\) => getTedPlayerContext\(\)\?\.sessionKey \|\| null/);
  assert.match(contentScript, /async buildTranscriptRequest\(\{ quietMissingPageData = false \} = \{\}\)/);
  assert.match(contentScript, /if \(!quietMissingPageData\) \{[\s\S]*Missing TED caption data/);
  assert.match(contentScript, /if \(handler\.platform === "ted"\) return;[\s\S]*tryRefresh\(\);/);
  assert.match(contentScript, /routeKey !== subtitleState\.currentRouteKey/);
  assert.match(contentScript, /setTranslationPhase\("temporary"\);[\s\S]*showOverlayMessage\(handler\.findVideo\(\), handler\.loadingMessage\)/);
