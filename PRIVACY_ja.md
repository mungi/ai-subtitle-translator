# AST - AI Subtitle Translator プライバシーポリシー

[한국어](PRIVACY.md) · [English](PRIVACY_en.md) · [日本語](PRIVACY_ja.md)

施行日: 2026年7月21日

AST - AI Subtitle Translator は、Udemy、YouTube、TED、NVIDIA Academy、Vimeo の字幕をユーザーが選択した翻訳サービスで翻訳する Chrome 拡張機能です。開発者は独自の backend server を運用せず、広告、追跡、分析を目的としてユーザーデータを収集しません。

## 取り扱うデータ

- Udemy、YouTube、TED、NVIDIA Academy、Vimeo の字幕テキストおよび cue の時間情報
...
- Host permissions: Udemy・YouTube・TED・NVIDIA Academy・Vimeo の字幕取得、選択した翻訳 provider の呼び出し、`localhost`/`127.0.0.1` の Custom LLM への接続に使用します。カスタム HTTPS Custom LLM domain には、モデル取得または接続テスト時にユーザーが許可した場合のみアクセスします。
...
プライバシーに関する質問とセキュリティ報告は [GitHub Issues](https://github.com/mungi/ai-subtitle-translator/issues) を利用してください。重要な変更はこの文書の施行日とリポジトリの変更履歴に反映します。
